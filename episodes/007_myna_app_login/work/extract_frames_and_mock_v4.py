# -*- coding: utf-8 -*-
"""draft_v4 代表フレーム抽出 + v3/v4 回帰確認 + CTA比較."""
...
DRAFT = BASE / "output" / "draft_v4.mp4"
OUT = BASE / "work" / "qa_frames_draft_v4"
V3 = BASE / "work" / "qa_frames_draft_v3"
...
for f in sorted(OUT.glob("scene_*.png")):
    v3f = V3 / f.name
    if not v3f.exists():
        diff_scenes.append((f.name, "no_v3"))
        continue
    if hashlib.sha256(f.read_bytes()).hexdigest() != hashlib.sha256(v3f.read_bytes()).hexdigest():
        diff_scenes.append((f.name, "DIFF"))
print("REGRESSION same_as_v3:", "PASS" if not diff_scenes else diff_scenes)
...
left = PImage.open(V3 / "cta.png").convert("RGB")
...
cd.text((20, 8), "v3 (draft_v3 CTA: ロゴあり)", font=f(30), fill="#ffffff")
cd.text((W2 + 20, 8), "v4 (draft_v4 CTA: ロゴなし)", font=f(30), fill="#ffffff")
cd.line((W2 - 1, 46, W2 - 1, comp.height), fill="#d4e5f0", width=2)
comp.save(REVIEW / "cta_v3_v4_comparison.png")
print("cta_v3_v4_comparison saved")
print("DONE")
