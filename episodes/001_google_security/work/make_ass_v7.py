...
Style: Default,Yu Gothic,62,&H00FFFFFF,&H00FFFFFF,&H00000000,&H96000000,-1,0,0,0,100,100,0,0,1,3,0,5,60,60,45,1
...
srt = (EP / "captions_v7.srt").read_text(encoding="utf-8")
blocks = re.split(r"\n\s*\n", srt.strip())
out = [HEADER]
n_split = 0
for b in blocks:
    ...
    for st, en, t in events:
        ...
        out.append(f"Dialogue: 0,{ts(st)},{ts(en)},Default,,0,0,45,,{{\\an5\\pos(960,990)}}{t}")
(EP / "captions_v7.ass").write_text("\n".join(out) + "\n", encoding="utf-8")
dlg = sum(1 for l in out if l.startswith("Dialogue:"))
print(f"captions_v7.ass: {dlg} cues / split={n_split} (62px, pos 960,990)")
